...
public abstract class Cell : MonoBehaviour, IGraphNode
{
	...
	public bool isWithinAbilityRange = false;
	...
	public GameObject returnCellUnit(Cell cell)
	{
		GameObject value = null;
		foreach (Avatar gameObj in Avatar.avatarList)
		{
			if(gameObj != null){
				if((gameObj.GetComponent<Avatar>() != null)){
					if(gameObj.transform.position.x == cell.transform.position.x && gameObj.transform.position.y == cell.transform.position.y){
						value = gameObj.gameObject;
	...
	public abstract void MarkAsAbilityReachable();

	public abstract void MarkAsAOE();
